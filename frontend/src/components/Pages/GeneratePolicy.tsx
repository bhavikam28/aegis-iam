...
import { Shield, Send, RefreshCw, User, Bot, MessageSquare, Lock, Zap, ArrowRight, CheckCircle, ChevronDown, ChevronUp, AlertCircle, Download, Copy } from 'lucide-react';
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 sm:pt-20 pb-16 sm:pb-32">
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-8 mt-8 sm:mt-12 text-slate-400 text-sm sm:text-base">
      ...
      {(isRefining || response) && (
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 sm:mb-12 gap-4">
            <div>
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2">Policy Generated</h2>
              <p className="text-slate-400 text-sm sm:text-base">Review and refine your secure IAM policy</p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 sm:gap-8">
            <div className="lg:col-span-2 space-y-4 sm:space-y-6">
                ...
                <div className={`p-4 sm:p-6 space-y-3 sm:space-y-4 overflow-y-auto transition-all duration-300 ${
                  ...
                  {chatHistory.length === 0 ? (
                    <div className="text-center py-6 sm:py-8 text-slate-400">
                      ...
                      <p className="text-sm sm:text-base">Start refining your policy by asking questions below</p>
                    </div>
                  ) : (
              ...
              <div className="bg-slate-900/50 backdrop-blur-xl border border-purple-500/20 rounded-xl sm:rounded-2xl p-4 sm:p-6">
                <label className="block text-white font-medium mb-2 sm:mb-3 text-sm sm:text-base">Refine Policy</label>
              ...
              {response?.refinement_suggestions && response.refinement_suggestions.length > 0 && (
                <div className="bg-slate-900/50 backdrop-blur-xl border border-purple-500/20 rounded-xl sm:rounded-2xl p-4 sm:p-6">
                  <h4 className="text-white font-medium mb-3 sm:mb-4 text-sm sm:text-base">Quick Refinements</h4>
                  <div className="space-y-2">
                    {response.refinement_suggestions.map((suggestion, index) => (
                      ...
                      >
                        {suggestion.startsWith('⚠️') || suggestion.toLowerCase().includes('warning') ? (
            ...
            <div className="lg:col-span-3 space-y-4 sm:space-y-6">
              {loading && !response ? (
                <div className="bg-slate-900/50 backdrop-blur-xl border border-purple-500/20 rounded-xl sm:rounded-2xl h-[400px] sm:h-[600px] flex items-center justify-center">
                  <div className="text-center max-w-md px-4">
                    <div className="relative mb-6">
                      <div className="w-16 h-16 sm:w-20 sm:h-20 border-4 border-slate-700 border-t-purple-500 rounded-full animate-spin mx-auto"></div>
                      <Shield className="w-6 h-6 sm:w-8 sm:h-8 text-purple-400 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
                    </div>
                    ...
                    <div className="mt-6 sm:mt-8 space-y-3 text-left">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-slate-300 text-xs sm:text-sm">Understanding requirements</span>
                      </div>
                      <div className="flex items-center space-x-3">
                        <div className="w-2 h-2 bg-purple-400 rounded-full animate-pulse"></div>
                        <span className="text-slate-300 text-xs sm:text-sm">Applying AWS best practices</span>
                      </div>
                      <div className="flex items-center space-x-3">
                        <div className="w-2 h-2 bg-slate-600 rounded-full"></div>
                        <span className="text-slate-500 text-xs sm:text-sm">Validating security posture</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <>
                    ...
                    {response.score_explanation && (
                      <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-slate-700">
                        <div className="text-slate-300 text-xs sm:text-sm leading-relaxed whitespace-pre-line">
                          {cleanMarkdown(response.score_explanation)}
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-slate-900/50 backdrop-blur-xl border border-purple-500/20 rounded-xl sm:rounded-2xl p-6 sm:p-8">
                    <h4 className="text-white text-base sm:text-lg font-semibold mb-3 sm:mb-4">Explanation</h4>
                  ...
                  {response.security_features && response.security_features.length > 0 && (
                    <div className="bg-purple-500/5 backdrop-blur-xl border border-purple-500/30 rounded-xl sm:rounded-2xl p-6 sm:p-8">
                      <h4 className="text-purple-400 text-base sm:text-lg font-semibold mb-3 sm:mb-4">Security Features</h4>
                        ...
                        {response.security_features.map((feature, index) => (
                          <li key={index} className="text-slate-300 text-xs sm:text-sm flex items-start space-x-2 sm:space-x-3">
                            {feature.startsWith('✅') ? (
                              ...
                              <AlertCircle className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400 mt-0.5 flex-shrink-0" />
                            ) : (
                              <Shield className="w-3 h-3 sm:w-4 sm:h-4 text-purple-400 mt-0.5 flex-shrink-0" />
                            )}
                  ...
                  {response.security_notes && response.security_notes.length > 0 && (
                    <div className="bg-orange-500/5 backdrop-blur-xl border border-orange-500/30 rounded-xl sm:rounded-2xl p-6 sm:p-8">
                      <h4 className="text-orange-400 text-base sm:text-lg font-semibold mb-3 sm:mb-4">Security Considerations</h4>
                      <ul className="space-y-2 sm:space-y-3">
