...
import { BarChart3, TrendingDown, ArrowRight, CheckCircle, Sparkles, Shield } from 'lucide-react';
import LoadingSpinner from '../UI/LoadingSpinner';
import CodeBlock from '../UI/CodeBlock';
import { AnalyzeHistoryRequest, AnalyzeHistoryResponse } from '../../types';
      ...
      <div className="relative max-w-7xl mx-auto px-8 py-16">
        ...
        <div className="mb-12">
          <div className="inline-flex items-center space-x-2 bg-orange-500/10 border border-orange-500/30 rounded-full px-6 py-2 mb-6">
            ...
            <span className="text-orange-300 text-sm font-medium">Usage Optimizer</span>
          </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {dateRanges.map(range => (
                      <button
                        key={range.value}
                        type="button"
                        onClick={() => setRequest({ ...request, date_range: range.value })}
                        className={`px-6 py-4 rounded-2xl font-medium transition-all ${
                          request.date_range === range.value
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-8">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-slate-400">Based on {request.date_range} days of CloudTrail analysis</p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-6">
                  ...
                  <div className="bg-gradient-to-br from-green-500/20 to-blue-500/20 rounded-2xl p-6 text-center border border-green-500/30">
            ...
            <div className="lg:col-span-2">
              <div className="space-y-6">
                ...
                <div className="bg-blue-500/10 backdrop-blur-xl border border-blue-500/30 rounded-2xl p-8">
                  <h4 className="text-blue-400 text-xl font-bold mb-4">Implementation Guide</h4>
                  <div className="space-y-4">
                    {response.implementation_steps.map((step, index) => (
                      <div key={index} className="flex items-start space-x-4">
                        <div className="w-8 h-8 bg-blue-500/20 text-blue-400 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">
                          {index + 1}
                        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6">
                <h4 className="text-white font-semibold mb-4">Impact Summary</h4>
                <div className="space-y-3 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400">Removed Permissions</span>
                    <span className="text-red-400 font-semibold">
                      {response.usage_summary.unused_permissions}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400">Retained Permissions</span>
                    <span className="text-green-400 font-semibold">
                      {response.usage_summary.used_permissions}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400">Attack Surface Reduction</span>
                    <span className="text-orange-400 font-semibold">
                      {Math.round((response.usage_summary.unused_permissions / response.usage_summary.total_permissions) * 100)}%
                    </span>
                  </div>
                </div>
              </div>
